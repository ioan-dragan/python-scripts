...
def write(fout, array):
    for i in range(0, len(array)):
        if ("Refutation not found" in str(array[i])):
            fout.write( " & RNF ")
        elif ("Time limit" in str(array[i])):
            fout.write( " & Timeout ")
        else: 
...
def printThisProblem(prb):
    if "Time limit" in prb.terminationReason[0]: 
        for idx in range(1,len(prb.terminationReason)):
            if not("Time limit" in prb.terminationReason[idx]) and not("NOT_FOUND" in prb.terminationReason[idx] ) and not("Refutation not found" in prb.terminationReason[idx]) and not("Memory limit" in prb.terminationReason[idx]):    
                return True
    return False
...
def updateStat(reasons):
    ...
    for i in range(0, len(reasons)):
        if not("Time limit" in reasons[i]) and not("NOT_FOUND" in reasons[i] ) and not("Refutation not found" in reasons[i]) and not("Memory limit" in reasons[i]):    
